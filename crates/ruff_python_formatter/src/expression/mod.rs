...
impl MaybeParenthesizeExpression<'_> {
    pub(crate) fn needs_parentheses(&self, context: &PyFormatContext) -> OptionalParentheses {
        ...
        let preserve_parentheses = parenthesize.is_optional()
            && is_expression_parenthesized((*expression).into(), context.source());
...
impl Format<PyFormatContext<'_>> for MaybeParenthesizeExpression<'_> {
    fn fmt(&self, f: &mut PyFormatter) -> FormatResult<()> {
        ...
        match self.needs_parentheses(f.context()) {
